import React from "react";

const AudioBooksCardMAking = ({ book }) => {
    const {_id, name, image_url, brand, description, product_code} =book;
    let id = book._id;
  return (
    <div>
      <div className="card w-96 bg-base-100 shadow-xl mx-auto justify-center">
        <figure>
          <img src={image_url} alt="Shoes" />
        </figure>
        <div className="card-body">
          <h2 className="card-title">{name}</h2>
          <div className="badge badge-secondary ">{brand}</div>
          <div className="badge badge-secondary">{product_code}</div>
        </div>

        {/* <Link to={`/details/${id}`}>
          <button className="btn block bg-green-600 text-white mx-auto w-full mt-4">
            Detail's
          </button>
        </Link> */}
        {/* <Link to={`/updateproduct/${id}`}>
              <button className="btn block mx-auto w-full mt-4">Update Now!</button>
            </Link> */}
      </div>
    </div>
  );
};

export default AudioBooksCardMAking;
